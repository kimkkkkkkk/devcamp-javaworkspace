...
public class MyCanvas extends Canvas{
	MoveTest moveTest; // 필요하면 보유하기
	
	//이 생성자를 호출하는 자는, MoveTest의 주소값을 
	//인수로 넘겨야 한다!!
	//이렇게 하면 , MyCanvas는 태어날때부터 MoveTest의
	//주소값을 보유하게 된다
	public MyCanvas(MoveTest moveTest) {
		this.moveTest=moveTest;
	}
	/*캔버스에 그려질 그림을 개발자가 주도하여, 원하는
	그림을 그려보자!!
	paint 메서드에 대하여...
	특징 : 개발자가 paint메서드를 호출해서는 안된다!!
			왜? 자바의 모든 컴포넌트는 스스로를 그린다.. 즉 paint 메서드는 시스템이 그래픽의 렌더링시
			필요에 의해 호출하게 됨..
			언제 호출됨?? 처음의 그림과 비교해서 바뀐 부분이 있다면, 시스템이 알아서 다시 호출
	 		...
	 		문제점? 	사용자가 윈도우창의 크기를 변화시킨다거나, 하는 그래픽상의 변화를 통해 paint()
	 					...
	 					sun 에서는 그래픽이 화면에 반영되는 처리는 시스템이 하기 때문에 paint()메서드를 호출을
	 					...
	 					화면을 지우며, 홤녀을 지우고 나서 깨끗해지면 paint()
	 					repaint() --> update() --> paint()
	 					
	 		*/
	public void paint(Graphics g) {
		g.setColor(Color.MAGENTA); // 이 시점부터는, 노란색으로 그려진다..
