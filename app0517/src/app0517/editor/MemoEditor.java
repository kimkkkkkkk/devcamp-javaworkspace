...
public class MemoEditor extends JFrame {
	...
	public MemoEditor() {
		...
		// 하지만, 클래스 중에는 재사용성이 별로 없는 1회성의 클래스도 있따.. 이런 경우 굳이 개발자가 .java파일을
		// 물리적으로 정의해가면서까지 개발할 필요가 있는가?? 따라서 sun에서는 클래스 안에 이름없는 클래스를 넣을 수 있도록
		// 내부 익명 클래스를 지원해주며, 주로 이벤트 구현시 압도적으로 많이 사용된다..
		// 또한 내부익명 클래스를 사용하면, 객체간 주소를 전달해야하는 불편함도 해소할 수 있다. 즉 내부익명 클래스는 외부클래스
		// 의 멤버들을 자기껏 처럼 접근할 수 있따.
		...
		item_saveas.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				/* 1) showSaveDialog() 호출하여 파일 저장 탐색기 띄운다
				 	2) 선택한 파일경로 얻는다
				 	3) 빈파일을 파일 출력스트림으로 생성한다
				 	4) 생성된 파일 출력스트림을 통해 area의 내용을 파일에 넣는다(출력)
				*/
		...
		setSize(700, 600);
		setVisible(true);
		setDefaultCloseOperation(EXIT_ON_CLOSE); // 무엇가 차을 닫을때, 자원을 반납하려면
	...
	public void openFile() {
		...
		if (res == JFileChooser.APPROVE_OPTION) {
			...
			// 문자(영문, 한글 모두: 즉 2byte 를 이해함) 를 읽을 수 잇는 스트림 생성
			FileReader reader = null; // close하기 위해 밖으로 뻄
			BufferedReader buffr = null;
			try {
				reader = new FileReader(file); // 파일을 대상으로 빨대 꽂은
				buffr = new BufferedReader(reader); // 버퍼처리된 스트림으로 빨대 업그레이드

				String data = null; // 읽혀진 문자(정확히는 문자의 코드)를 담게 될 정수

				while (true) {
					data = buffr.readLine(); // 한 문자열 즉 한 줄 (캐릭터들을 모아서 ) 읽기
					if (data == null)
						break;
					area.append(data+"\n");
				}
