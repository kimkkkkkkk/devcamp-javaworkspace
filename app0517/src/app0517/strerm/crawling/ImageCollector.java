package app0517.strerm.crawling;
...
public class ImageCollector {
	...
	public ImageCollector() { 
		...
		try {
			...
			http=(HttpURLConnection)con; // 다운 캐스팅!! 같은 자료형이므로 형변환 가능!!
			http.setRequestMethod("GET"); //GET방식의 요청!!
			
			
			//웹서버에 요청을 시도해보자!!
			is=http.getInputStream(); // 웹서버와의 요청시 생성된 입력스트림
			fos = new FileOutputStream("C:\\korea202102_javaworkspace\\app0517\\res\\target.jpg"); // 복사될 파일 즉 생성될 파일의 경로

			long startTime=System.currentTimeMillis(); //현재 시간을 밀리 second로 반환
			//이 입력스트림으로 서버의 자원을 가져올 수 있따.
			int data=-1;
			...
			System.out.println("인터넷상의 이미지를 수집완료했씁니다. 디렉토리를 확인해보세요");
