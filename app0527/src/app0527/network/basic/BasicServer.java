...
public class BasicServer {
	...
	//네트워크 프로그램시 항상 염두해두어야 할 점은 네트워크 프로그램의 원리가 전화의 워니와 상당히
	//흡사한다는 점임..
	...
	public BasicServer() {
		try {
			...
			//접속자가 발견되면, 이 시점부터는 접속자인 클라이언트와 대화를 나눌 수 있으므로, 대화를 주고 받는 전화기에 해당하는 소캣을
			//반환받는다!!
			...
			data = is.read(); // 소켓과 연결된 입력스트림으로부터, 클라이언트의 메시지 중 1byte를 읽어들임
									//즉 말이 네트워크 프로그램이지, 우리가 연결한 대상이 단지 네트워크일 뿐이며
									//실제적인 프로그램은 스트림 제어다
