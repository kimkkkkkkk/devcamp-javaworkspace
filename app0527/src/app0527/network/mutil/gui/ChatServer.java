...
public class ChatServer extends JFrame implements ActionListener{
	...
	Thread thread; // 접속자의 감지를 처리하기 위한 쓰레드, 얘가 없다면? 메인 쓰레드가 대기상태에 빠진다..
	...
	public void startServer() {
		...
		try {
				...
				Socket socket=server.accept(); // 이 코드에 의해 접속자가 발견될때까지 실행부는 무한대기에 빠진다.. 특히나 무한대기에 빠지게 됨
				//실행부가 메인쓰레드라면, 메인 쓰레드의 고유 업무인 이벤트 처리, gui 처리 등을 할 수 가 없다!! 따라서 프로그램이 먹통된다!!
				...
				msgThread.start(); // 각각 알아서 움직여라!! jvm님이 알아서 떄려줌
