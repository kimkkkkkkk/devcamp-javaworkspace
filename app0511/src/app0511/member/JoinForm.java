...
public class JoinForm extends JFrame implements ActionListener{
	...
	public JoinForm() {
		...
		p_hobby = new JPanel();
		ch_hobby = new JCheckBox[5]; // 빈공간만 생성함
		...
		la_receive.setPreferredSize(new Dimension(190,30)); // 메일 수신 라벨
	...
	public void checkValue() {
		...
		if(t_id.getText().length() == 0) {
			...
			t_id.requestFocus();//커서 올려 놓기
		}else if(data.length()<8) { // 8자 이하라면
			JOptionPane.showMessageDialog(this, "비밀번호는 8자이상 입력하세요");
			t_pass.requestFocus();//커서 올려 놓기
		}else if(count<1){
			JOptionPane.showMessageDialog(this, "적어도 1개 이상은 선택하세요");
		}else {
			//오라클 DB입력
		}
	}
