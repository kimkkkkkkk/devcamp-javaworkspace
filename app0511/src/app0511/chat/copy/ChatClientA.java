...
public class ChatClientA extends JFrame implements ActionListener, KeyListener{
	...
	JTextField t_input;
	JButton bt;
	ChatClientB chatClientB; //내가 제어하고 싶은 상대방 창에 대한 래퍼런스 변수
	...
	public ChatClientA() {
		...
		//이벤트 리스너와의 연결
		//특히 이벤트리스너가 연결된 컴포넌트를 가리켜 이벤트
		bt.addActionListener(this); // 전송버튼과 리스너 연결
		bt_open.addActionListener(this); //오픈 버튼과 리스너 연결 , new는 하지 않고 기존의 것 (위) 쓰기
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == bt_open) {
			...
			chatClientB = new ChatClientB(); // b를 태어나게 하고
