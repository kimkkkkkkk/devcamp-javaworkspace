...
public class GamePanel extends JPanel{
	...
	public static final int  HELGHT=600;
	...
	public void createBg() {
		ImageIcon icon = imageManager.getScaledIcon("bg.jpg", WIDTH, HELGHT);
	...
	public void createEnemy() {
		//(GamePanel gamePanel, String path,int x, int y, int width, int height, int velX, int velY)
	...
	protected void paintComponent(Graphics g) {
		g.drawImage(bgImg, 0, 0,WIDTH, HELGHT, this);
		hero.render(g);
	...
	public void gameLoop() {
		//게임에 등장하는 모든 오브젝트를 움직이게하려면, tick(), render() 호출
		hero.tick();
		for(int i=0;i<enemyArray.length;i++) {
			enemyArray[i].tick();
		}
