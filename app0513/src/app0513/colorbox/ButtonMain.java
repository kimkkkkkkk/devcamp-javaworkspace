...
public class ButtonMain extends JFrame implements ActionListener{
	...
	//불편하다..그렇다면 자바스크립트처럼 배열의 크기가 동적으로 늘어날 수 있는 형태의 객체는 없을까? 당연히 있다.
	//JButton[] btnArray2 = new JButton[10]; // 낱개로 생성된 버튼들을 담게될 배열
	...
	//사용자가 입력한 갯수만큼 버튼을 생성한다!!
	public void create() {
		//(1)텍스트 필드 값 구하기
		int ea = Integer.parseInt(t_ea.getText()); // "" 벗겨내기;
		btnArray=new JButton[ea];//배열의 크기를 결정지을 수 있다.. 따라서 이때 배열을 생성할 수 있따..
		
		//(2)갯수만큼 버튼을 생성하여 p_center에 붙이기
		for(int i=0;i<ea;i++) {
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == bt_create) { //일괄생성
			create();
		}else if(e.getSource() == bt_create2) { //낱개 생성
			createOne();
