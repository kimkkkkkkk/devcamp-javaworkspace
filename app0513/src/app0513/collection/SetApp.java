...
public class SetApp {
	...
	public static void main(String[] args) {
		...
		set.add("banana"); // List와 동일하게 add() 메서드가 지원되는 이유는? 모두~ Collection객체의 자식이기 떄문이다.
		set.add("apple");
		set.add("mango");
		//결론 : Set은 데이터 중복을 허용하지 않음
		//순서가 없는 데이터형으로, 일렬로 늘어서게 해	ㅗ자
		Iterator <String> it = set.iterator();
		
		//반복문을 이용해서 모든 요소 출력하기!!
		while (it.hasNext()) { // 현재위치를 기준으로, 다음 위치에 요소가 존재하는 동안만 반복문 수행
